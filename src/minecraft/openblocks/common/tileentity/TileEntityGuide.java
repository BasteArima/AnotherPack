...
public class TileEntityGuide extends NetworkedTileEntity implements IShapeable,
	...
	public void switchMode(EntityPlayer player) {
		switchMode();
		if (player != null) {
			player.sendChatToPlayer(String.format("Переход в режим %s", getCurrentMode().getDisplayName()));
		}
	}
	...
	public void changeDimensions(EntityPlayer player, ForgeDirection orientation) {
		...
		player.sendChatToPlayer(String.format("Размер изменен на %sx%sx%s", width.getValue(), height.getValue(), depth.getValue()));
	}

	public void changeDimensions(ForgeDirection orientation) {
		if (width.getValue() > 0 && orientation == ForgeDirection.EAST) {
			width.modify(-1);
		} else if (orientation == ForgeDirection.WEST && width.getValue() < 15) {
			width.modify(1);
		} else if (orientation == ForgeDirection.NORTH && depth.getValue() < 15) {
			depth.modify(1);
		} else if (depth.getValue() > 0 && orientation == ForgeDirection.SOUTH) {
			depth.modify(-1);
		} else if (orientation == ForgeDirection.UP && height.getValue() < 15) {
			height.modify(1);
